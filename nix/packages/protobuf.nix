# protobuf -- BLOCKED. cmake, ~221 TUs across the runtime library + protoc
# + the `gtest`/`abseil-cpp`-backed test suite. Flagged ahead of time as
# the riskiest survey candidate (heaviest cmake build attempted in this
# repo, real cross-package build deps: gtest, zlib, abseil-cpp pulled in
# via `-Dprotobuf_ABSL_PROVIDER:STRING=package`). Tried anyway per
# explicit instruction; hits a real, previously-known dyn-drvs bug, but
# at a scale/consequence this repo hadn't exercised before.
#
...
#    freshly cc/c++-linked executable comes out of the sandbox missing
#    its execute bit). protobuf's own CMake build runs its own
...
#    any.proto, api.proto, duration.proto, hundreds of test .proto files,
#    etc.) into its corresponding `.pb.h`/`.upb.h`/`.upbdefs.h`/
#    `.upb_minitable.h`. Every one of those generation steps fails
#    identically:
#
#      /nix/store/.../bash: line 1: ./protoc: Permission denied
#      make[2]: *** [CMakeFiles/upb-test.dir/build.make:85: google/protobuf/any.upbdefs.h] Error 126
#
#    (repeated for descriptor/api/duration/edition_unittest/
#    map_proto2_unittest/test_bad_identifiers_*/test_large_enum_value/
#    internal_metadata_locator_test and more -- dozens of distinct
#    `.proto` targets, all Error 126, all "Permission denied" on the
#    same `./protoc`), which cascades into
#    `CMakeFiles/upb-test.dir/all`/`CMakeFiles/libtest_common.dir/all`
#    failing and the whole `make` aborting with `Error 2`.
#
