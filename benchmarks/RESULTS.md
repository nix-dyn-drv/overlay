...
| re2 (~51 TUs, cmake+ninja) | cold build | all TUs compiled+linked | **PASS** (fixed by dyn-drvs 97a987d + 0d233d3) | -- | Originally reported as a distinct bug in the wider survey below: 20 of ~51 compile-unit derivations failed with `cc1plus: fatal error: <src>.cc: No such file or directory` at the *compile* step (not link), for real primary source files. Retested and precisely isolated by pinning `dyndrv` at successive commits: **FIXED by dyn-drvs 97a987d** alone -- confirmed this WAS the exact same discoverTree cmake-source-path bug already fixed for xxHash/tinycbor/leveldb/brotli, cmake+ninja vs cmake+make was never the distinguishing factor. Then blocked by a cmake+ninja-specific variant of the install-time regeneration bug: `CMake Error: The source directory "/build/source" does not appear to contain CMakeLists.txt` / `ninja: error: rebuilding 'build.ninja': subcommand failed`. **FIXED by dyn-drvs 0d233d3** ("Fix phases.split cmake+make out-of-tree install failure (task #138)") -- confirmed this fix's own `dyndrvCdToBuildDir` reconstruction generalizes to ninja's own `--regenerate-during-build` step, the exact same shape as cmake+make's `cmake_check_build_system`. Confirmed PASS with both fixes together: real `libre2.so.11.0.0` verified as a genuine ELF shared object, no package-level workaround needed. Details in `nix/packages/re2.nix`. |
...
- **re2: PASS, fixed upstream (dyn-drvs 97a987d + 0d233d3).** Wired into
  this flake as `dyndrv-re2` -- see table above. Originally 20 of ~51
  compile-unit derivations failed with `cc1plus: fatal error:
  <src>.cc: No such file or directory` at the *compile* step (not link),
  for real primary source files, reported as a distinct, new bug at the
  time. Retested and isolated precisely: `97a987d` (the same
  discoverTree `-MT`/`-MF` fix that already fixed the other four
  cmake-source-path instances) fixes this exactly -- it WAS the same
  bug, cmake+ninja vs cmake+make was never the distinguishing factor.
  Then blocked by a cmake+ninja-specific variant of the cmake+make
  out-of-tree install bug (`ninja: error: rebuilding 'build.ninja':
  subcommand failed`, `CMake Error: The source directory "/build/source"
  does not appear to contain CMakeLists.txt`) -- fixed by `0d233d3`
  (the SAME fix that resolved capnproto's cmake+make variant,
  generalizing to ninja's own regeneration step too). Confirmed PASS
  with both fixes: real `libre2.so.11.0.0` verified as a genuine ELF
  shared object, no package-level workaround needed. Details in
  `nix/packages/re2.nix`.
