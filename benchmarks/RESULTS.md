...
| x265 (~99 TUs) | cold build | **BLOCKED** | -- | -- | Originally reported INCONCLUSIVE (a pre-existing plain-nixpkgs nasm `label-redef-late` error assembling `common/x86/intrapred16.asm`, confirmed by an A/B rebuild independent of `mkAcceleratedStdenv`). Retested and that nasm error did NOT recur -- all ~92 real `.asm.o` files (including `intrapred16.asm`) now assemble successfully. Now genuinely BLOCKED by a real dyn-drvs bug: archiving into `libx265_a.a`/`libhdr10plus_a.a` via `ar`/`ranlib` fails with `ar: /nix/store/<hash>-analysis.cpp.o: No such file or directory` / `ranlib: '.../dyndrv-libx265_a': No such file` (same shape for `dyndrv-libhdr10plus_a` with `json11.cpp.o`). The referenced `.o` files genuinely exist as valid ELF objects on disk; `nix derivation show` confirms `inputs.drvs = {}` for both failing derivations -- the `.o` files are never wired as real build dependencies. Same bug class as libwebp/openjpeg (`ar`/`ranlib`'s own positional `.o` inputs not declared as `inputs.drvs`). Details in `nix/packages/x265.nix`. |
