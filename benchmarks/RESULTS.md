...
| leveldb (~39 real TUs) | cold build | **BLOCKED** | -- | -- | Every single one of the 39 real per-TU compiles fails identically: `cc1plus: fatal error: db/c.cc: No such file or directory` (same for every other `db/*.cc`/`table/*.cc`/`util/*.cc`, 39/39, a clean 100%). Same discoverTree cmake-source-path bug as xxHash/re2/tinycbor, confirmed a fourth time -- and notably leveldb's `CMakeLists.txt` lives at the repo root (no out-of-tree `build/cmake` subdir like xxHash), so "out-of-tree cmake dir" is confirmed not a precondition; a same-tree cmake+make project hits it just as universally, and with a higher failure rate than re2's partial 20/51. No package-level workaround. Details in `nix/packages/leveldb.nix`. |
