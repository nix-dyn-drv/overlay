...
| x265 (~99 TUs) | cold build | **INCONCLUSIVE** | -- | -- | Not a dyn-drvs bug: `pkgs.x265` (4.2) fails to build PLAIN on this flake's own nixpkgs pin, with or without `mkAcceleratedStdenv` -- confirmed by a direct A/B rebuild, same nasm error either way (`nasm: ... error: label 'x265_10bit_intra_pred_ang32_31_sse4.loop' changed during code generation ... [-w+error=label-redef-late]` while assembling `common/x86/intrapred16.asm`). `mkAcceleratedStdenv` never wraps `nasm`, only `cc`/`c++`/`ar`/`ranlib`, so acceleration cannot be involved. Genuinely can't evaluate this package against dyn-drvs until the underlying nixpkgs/nasm-3.02 bug is fixed upstream. Details in `nix/packages/x265.nix`. |
