...
| libwebp (~171 TUs) | cold build | **BLOCKED** | -- | -- | Single-output, cmake build -- picked specifically to dodge the multi-output gaps (openssl/libpng), but hits the exact same discoverTree cmake-source-path bug as tinycbor/xxHash/re2: every real TU compile across every one of libwebp's cmake targets fails identically, e.g. `cc1: fatal error: /build/source/examples/dwebp.c: No such file or directory`. `nix derivation show` + inspecting the failing TU's staged `dyndrv-tree` shows only `src/` materialized (plus an empty `CMakeFiles/dwebp.dir/examples/` skeleton) and a literal top-level `-I/build/source` directory -- discoverTree appears to have misparsed one of the `-I<path>` flags as a file path to stage rather than resolving the actual `-c` source argument, so `examples/`, `imageio/`, `sharpyuv/`, `src/mux/`, `src/dec/`, `src/demux/` never get staged. A fourth confirmed instance of the same open bug, on yet another real source-tree shape. See `nix/packages/libwebp.nix`. |
...
- **tinycbor: same cmake-source-path bug against this repo's actual
  ...
  single-output) independently confirms the same bug a fourth time --
  wired in as `dyndrv-libwebp`, also BLOCKED (see table above).
